...
def game_4_players():
    ...
    game = Game(4, ["Alice", "Bob", "Charlie", "David"])
    game.start_game()
...
class TestBidding:
    ...
    def test_player_removal_on_pass(self, game_3_players):
        """Test that players are removed from auction on pass. Check for correct next player"""
        game = game_3_players
        bid_handler = BidHandler(0, 3)

        game.set_current_player(1)
        assert len(bid_handler.remaining_players) == 3

        bid_handler.pass_bid(2)
        assert len(bid_handler.remaining_players) == 2
        assert 2 not in bid_handler.remaining_players
        assert [0, 1] in bid_handler.remaining_players

        bid_handler.pass_bid(0)
        assert len(bid_handler.remaining_players) == 1
        assert 0 not in bid_handler.remaining_players
    ...
    def test_buy_back_action(self, game_3_players):
        ...
        assert (
            len(game.get_player(bid_master).get_cow_inventory())
            == bid_master_initial_cows + 1
        )
...
class TestGameFlow:
    ...
    def test_bid_flow_end_to_end(self, game_4_players):
        ...
        p0 = make_player_interface_mock(0, bid_value=None, buy_back=True)  # bid master
        p1 = make_player_interface_mock(1, bid_value=10)  #
        p2 = make_player_interface_mock(2, bid_value=40)  #
        p3 = make_player_interface_mock(3, bid_value=None)  #
        player_interfaces = [p0, p1, p2, p3]
        ...
        assert game.get_player(buyer_idx).get_optimal_payment(40) == [
            0,
            0,
            1,
            0,
            0,
            0,
        ]  # overpay as starting money is too low
        assert game.get_player(buyer_idx).get_cow_inventory() == [10]
        assert 10 not in game.get_player(seller_idx).get_cow_inventory()
        assert (
            game.get_player(buyer_idx).get_money_value()
            < game.get_player(seller_idx).get_money_value()
        )
        assert (
            game.get_player(seller_idx).get_money_value()
            == game.get_money_value(seller_initial_money) + 50
        )
