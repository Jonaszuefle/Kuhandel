...
class InputHandler(ABC):
    def __init__(self, player_names: dict[int: str]):
        ...
        self.name_to_idx = {name.lower(): idx for idx, name in self.player_names.items()}
...
class ConsoleInputHandler(InputHandler):                                # TODO write parsers and validators
    ...
    def _parse_idx_or_name(self, raw: str) -> tuple[Result, int]:
        lower_names = [names.lower() for names in self.player_names.values()]
        try:
            if raw in ["0","1","2","3","4"]:      # TODO remove hardcode          
                return Result(ResultType.SUCCESS), int(raw)
            elif raw.lower() in lower_names:
                return Result(ResultType.SUCCESS), self.name_to_idx[raw.lower()]
            else:
    ...
    def _validate_positive_int(self, value: int) -> Result:
        return Result(ResultType.SUCCESS) if value >= 0 else Result(ResultType.FAILURE, "Value must be positive.")
    
    def _validate_player_idx(self, idx: int) -> Result:
        if idx in self.player_names.keys():
