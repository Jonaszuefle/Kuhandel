...
class PlayTradeTurn:
    ...
    def execute(self):
        ...
        if not self.game.get_player(enemy_idx).has_cow(cow_type, cow_amount):
            return Result(ResultType.FAILURE, f"Challenged player does not have {cow_amount} of cow {cow_type}")
        
        if not self.game.get_current_player().has_cow(cow_type, cow_amount):
            return Result(ResultType.FAILURE, f"Current player does not have {cow_amount} of cow {cow_type}")
        ...
        if not self.game.get_current_player().has_enough_money(trade_contender.amount):
            return Result(ResultType.FAILURE, f"Challenged player does not have enough money")
