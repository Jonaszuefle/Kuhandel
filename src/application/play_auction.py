...
class PlayBidTurn:
    ...
    def execute(self):
        ...
        self.bid_handler = BidHandler(
            self.game.get_current_player_idx(), self.game.num_players
        )  # TODO write getter

        # Auction loop
        while True:
            for pl_idx in self.bid_handler.remaining_players:
                bid = self.player_interfaces[pl_idx].make_bid_decision(
                    self.game.get_player_view(pl_idx), self.bid_handler
                )

                if bid.value is None:  # player wants to pass
                    self.bid_handler.pass_bid(bid.player_idx)
                else:
                    result = self.bid_handler.place_bid(
                        self.game.get_player(pl_idx), bid.value
                    )
        ...
        if winner_bid.value < self.game.get_current_player().get_money_value():
            wants_buy_back = self.player_interfaces[
                self.bid_handler.bid_master
            ].make_buy_back_decision(winner_bid)
        else:
