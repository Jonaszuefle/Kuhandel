...
class Player():
    ...
    def update_score(self):
        ...
        self.__score = sum(self.cow_cards_obj.cow_finished) * 4 * len(self.cow_cards_obj.cow_finished)
...
class MoneyCards():
    def __init__(self):
        self.__money_inventory = GameConfig.STARTING_MONEY   # amount of 0, 10, 50, 100, 200, 500

    def get_money_inventory(self):
        return self.__money_inventory

    def add_money(self, money_list: list[int]):
        self.__money_inventory = [x + y for x, y in zip(self.__money_inventory, money_list)]
    ...
    def return_money_value(self):
        return sum([a*b for a,b in zip(self.__money_inventory, GameConfig.MONEY_CARD_VALUES)])
